  ...
  const fetchUserData = async () => {
    ...
    if (user) {
      try {
        ...
        const q = query(parentRef, where('email', '==', user.email));
        const querySnapshot = await getDocs(q);

        if (!querySnapshot.empty) {
          const parentDoc = querySnapshot.docs[0]; // Get the first matching document
          const parentData = parentDoc.data();

          setUserName(parentData.username || '');
          setEmail(parentData.email || '');
          setContactNumber(parentData.contactNumber || '');
        } else {
          Alert.alert('Error', 'User not found in parent collection');
        }
      } catch (error) {
  ...
  const handleSaveChanges = async () => {
    ...
    if (user) {
      try {
        ...
        if (!querySnapshot.empty) {
          const parentDocRef = querySnapshot.docs[0].ref; // Get the document reference
  ...
  return (
    <LinearGradient colors={['#BCE5FF', '#FFFFFF']} style={styles.container}>
      ...
      <LinearGradient colors={['#BCE5FF', '#FFFFFF']} style={styles.profileContainer}>
        ...
        <Text style={styles.infoid}>{contactNumber}</Text>
      ...
      >
        <View style={styles.modalContainer}>
          <View style={styles.modalContent}>
            ...
            <TextInput style={styles.input} placeholder="Phone" value={contactNumber} onChangeText={setContactNumber} />
